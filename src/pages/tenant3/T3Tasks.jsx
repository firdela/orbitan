...
const STATUS_MAP = {
  ...
  completed:   { label: 'Completed',   icon: CheckCircle2, color: 'text-[#22C55E]',         bg: 'bg-[#F0FDF4]' },
...
const CAT_COLORS = {
  ...
  Reporting:  'bg-purple-50 text-purple-700',
  ...
  return (
    <AppShell navigation={T3_NAV} tenant={T3_TENANT} title="" headerRight={<TenantSwitcher />}>
      <div className="p-4 sm:p-6 max-w-4xl mx-auto space-y-6">
        <PageHeader
          title="Tasks"
          subtitle="Renewed Fashion · Operational assignments & store tasks"
          actions={
            <Button size="sm" className="gap-2" style={{ background: '#22C55E' }} onClick={() => setShowNew(true)}>
        ...
        {urgentCount > 0 && (
          <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-center gap-3">
            ...
            <p className="text-sm font-medium text-red-700">{urgentCount} urgent task{urgentCount > 1 ? 's' : ''} need immediate attention.</p>
          </div>
        )}
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <select className="px-3 py-2 text-sm border border-border rounded-lg bg-background" value={filterCat} onChange={e => setFilterCat(e.target.value)}>
        ...
        <div className="space-y-2">
              ...
              <div key={task.id} className="bg-card border border-border rounded-xl p-4 flex items-start gap-3 hover:shadow-sm transition-shadow cursor-pointer" onClick={() => setSelectedTask(task)}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2 mb-1.5">
                    <p className={`text-sm font-medium leading-snug ${task.status === 'completed' ? 'line-through text-muted-foreground' : 'text-foreground'}`}>{task.title}</p>
                    <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border flex-shrink-0 whitespace-nowrap ${pc.bg} ${pc.color} ${pc.border}`}>{pc.label}</span>
                  </div>
          ...
          {filtered.length === 0 && (
            <div className="text-center py-12 text-muted-foreground">
              <CheckCircle2 className="w-10 h-10 mx-auto mb-3 text-[#22C55E] opacity-40" />
              <p className="text-sm">All clear — no tasks here.</p>
            </div>
          )}
        </div>
      </div>
          ...
          <div className="bg-white w-full max-w-sm h-full overflow-y-auto shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex items-center gap-2 text-muted-foreground"><User className="w-4 h-4" /><span>Assigned to <strong className="text-foreground">{selectedTask.assigned_to_name}</strong></span></div>
                <div className="flex items-center gap-2 text-muted-foreground"><Calendar className="w-4 h-4" /><span>Due <strong className="text-foreground">{selectedTask.due_date}</strong></span></div>
              </div>
              {selectedTask.notes && (
              ...
              <div className="flex gap-2 pt-2">
                ...
                <Button size="sm" className="flex-1" style={{ background: '#22C55E' }}>Mark Complete</Button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div className="flex gap-3 px-6 py-4 border-t border-border">
              ...
              <Button className="flex-1" style={{ background: '#22C55E' }} onClick={() => setShowNew(false)}>Create Task</Button>
            </div>
          </div>
        </div>
      )}
