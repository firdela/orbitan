...
## 22. Membership Provider Ownership Boundary (M3.2)

> The Membership Provider is the sole boundary responsible for converting
> external identity sources into the internal Membership contract. All
> downstream authorization components consume Membership only and remain
> independent of the underlying identity implementation.

The `MembershipProvider` contract (`resolve(identity, context) → Membership | null`,
frozen at **v1.0**) is the single seam between identity retrieval and
authorization evaluation. The pipeline depends on this contract alone and
never imports the Base44 SDK or inspects `Employee` fields.

- `EmployeeBase44Provider` is the **only** access-layer component that imports
  the Base44 SDK, queries the `Employee` entity, or understands the legacy
  Employee schema. It uses `translateEmployee` (the Compatibility Matrix
  adapter) and returns a normalized `Membership`.
- `MockMembershipProvider` is a pure, SDK-free implementation used by tests and
  as the reference shape for future providers (backend function, Microsoft
  Entra, Google Workspace, LDAP, future identity service).
- Fail-closed: provider throw → `provider_error`; null → `no_membership`;
  invalid shape → `invalid_membership`; inactive membership →
  `membership_inactive`. Two new denial reasons (`provider_error`,
  `invalid_membership`) extend the §7 vocabulary additively.

Swapping the identity source is a one-line provider change — nothing in the
Access Engine changes. RLS and tenant isolation are an implementation detail
of `EmployeeBase44Provider`, not an architectural dependency of the pipeline.
See `implementation-notes/access-engine-milestone-3.md` for the full M3.1/3.2
component map and test coverage.