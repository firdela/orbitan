...
18. **Find My Solution AI Diagnostic Wizard (2026-07-10):** Implemented AI-powered diagnostic wizard as step 0 of OnboardingWizard. 5 stages: Intent → Compliance → Industry → Scale → AI Prescription. Uses InvokeLLM (claude_sonnet_4_6) with response_json_schema to generate a ProvisioningManifest (pack, plan, modules, compliance_templates, governance_domain, rationale). Prescription pre-fills the existing wizard — user can accept or adjust manually. Diagnostic is skippable ("Skip — Choose Manually" footer). Compliance stage ensures regulated industries (F&B, healthcare, financial, environmental) trigger governance domain binding. ADR-0015 documents this decision. No entity changes, no backend changes — the prescription just tells the frontend which pack/plan/modules to pre-select. The existing onboardingService provisioning engine is unchanged.
...
6. ~~**No payment integration**~~ ✅ FIXED — Stripe checkout is now LIVE (Live Mode). Four subscription products in Stripe: OrbitanOS Starter (S$49/mo, prod_UqxyqWSPqzbmUU), OrbitanOS Growth (S$149/mo, prod_UqCTflVC5RIRyA), OrbitanOS Business (S$399/mo, prod_UqCTbi0NRW3noE), OrbitanOS Enterprise (S$1000/mo placeholder, prod_UqzfnTh7fdnvTs). `stripeCheckout` backend function creates Checkout Sessions for Starter + Growth + Business (all self-service). Enterprise is "Contact Sales" in the Checkout UI (not wired to self-service checkout). `stripeWebhook` handles `checkout.session.completed`, `customer.subscription.deleted`, `customer.subscription.updated`. Checkout page at `/checkout` shows all 3 self-service plans + Free tier with correct pricing matching Stripe. `orbitan-config.js` SUBSCRIPTION_PLANS now carries `stripe_product_id` + `stripe_price_id` on all 4 paid plans as the internal source of truth. Free tier has no Stripe product (price_sgd: 0, no checkout). Test card: 4242 4242 4242 4242.
...
- **Full Stripe Plan Alignment (2026-07-09, Session 2):** Completed end-to-end subscription configuration alignment. **Added Starter to self-service checkout** — `stripeCheckout` PRICE_MAP now includes `orbitan_starter: 'price_1TrG2gDap39FEFGJZXTY5or7'` (was previously excluded as "free"). All 3 self-service plans tested and confirmed returning `cs_live_` Stripe checkout URLs. **Added `stripe_product_id` + `stripe_price_id` to `orbitan-config.js` SUBSCRIPTION_PLANS** for all 4 paid plans — this is now the internal source of truth for Stripe ID mapping. Enterprise has `checkout_mode: "contact_sales"` (not wired to self-service checkout). **Updated Checkout.jsx** — added Starter plan card (3-column grid), updated all feature lists to match Master Vision exactly (Growth: "Core Workforce Package, One Industry Pack, Standard AI features"; Business: "Core Workforce Package, Multiple Industry Packs, Advanced reporting, AI Suite access, Priority support"). Enterprise remains "Contact Sales" CTA. Enterprise Stripe product exists at S$1000/mo but is NOT used for self-service checkout — custom pricing (S$1,999+) is handled via manual sales process per Master Vision.
- **Orbitan Free Tier Added (2026-07-09, Session 3):** Added the `orbitan_free` plan per Master Vision — "Remove barriers to entry." Config: `orbitan-config.js` SUBSCRIPTION_PLANS now has 5 plans (Free, Starter, Growth, Business, Enterprise). Free: S$0, 3 employees, basic attendance/tasks/training, 1 outlet, no AI/integrations/packs, slate-grey colour (#64748B). No Stripe product (not in PRICE_MAP, not checkoutable). **Tenant entity** updated — `subscription_plan` enum now includes `orbitan_free`. **Checkout.jsx** — Free card added as first plan (4-column grid: Free, Starter, Growth, Business), "Get Started Free" button routes to `/onboarding` (no Stripe call). **Landing.jsx** — PLAN_ORDER updated to include `orbitan_free`, grid changed to 5-column on lg. Free tier auto-renders via existing dynamic template. SubscriptionPolicy + OrbitanWallet entities already had `orbitan_free` in their enums — no changes needed there.
